...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        #step 3
        cells = set()
        count_value = copy.deepcopy(count)
        close_cells = self.return_close_cells(cell)
        for cel in close_cells:
            if cel in self.mines:
                count_value-= 1
            if cel not in self.mines and cel not in self.safes:
                cells.add(cel)                           
        new_sentence = Sentence(cells,count_value)   
    ...
    def make_safe_move(self):
        ...
        """
        for safe_row in range(self.height):
            for safe_column in range(self.width):
                if (safe_row,safe_column) in self.safes and (safe_row,safe_column) not in self.mines and (safe_row,safe_column) not in self.moves_made:
